...
class TrajectoryComputer:
    ...
    def set_initial_state_vectors(self, r_vec, v_vec):
        self.state = np.array(r_vec + v_vec, dtype=float)
        self.t = 0.0
    ...
    def _append_history_point(self):
        ...
        self.history["x"].append(float(x))
        self.history["y"].append(float(y))
        self.history["z"].append(float(z))
        self.history["r"].append(float(r))
    ...
    def _acceleration(self, state, t):
        ...
        a_grav = -(GM / r_norm**3) * r_vec if r_norm != 0.0 else np.zeros(3)
    ...
    def step(self, dt):
        ...
        substeps = 10  # Reduced from 50 to speed up while maintaining stability
        dt_sub = dt / substeps
